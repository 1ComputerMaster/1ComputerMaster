...
public class BoardController {
	...
	@GetMapping("/high")
	public ResponseEntity<List<BoardDto>> HighId() throws Exception {
		logger.info("HighId - 호출");
		return new ResponseEntity<List<BoardDto>>(boardService.HighId(), HttpStatus.OK);
	}
	
	@ApiOperation(value = "게시판 글보기", notes = "글번호에 해당하는 게시글의 정보를 반환한다.", response = BoardDto.class)
	@GetMapping("/{boardId}")
	public ResponseEntity<BoardDto> getArticle(@PathVariable("boardId") @ApiParam(value = "얻어올 글의 글번호.", required = true) int boardId) throws Exception {
		logger.info("getArticle - 호출 : " + boardId);
	...
	public ResponseEntity<String> deleteArticle(@PathVariable("boardId") @ApiParam(value = "삭제할 글의 글번호.", required = true) int boardId) throws Exception {
